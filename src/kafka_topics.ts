export default {
    ...
    generateTopicForTransaction(transactionId: string, topic: string) {
        return `saga-tx-${transactionId}-${topic}`;
    },
    matchTopicForTransaction(topic: string) {
        const re = new RegExp('saga-tx-.+-(.+)');
        const captures = topic.match(re);
        if (captures) {
            return captures[1];
        }
        return undefined;
